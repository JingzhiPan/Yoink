...
const api = {
  ...
  outline: (jobId: string, id: string, brief: string): Promise<PatternMeta> => ipcRenderer.invoke('pipeline:outline', jobId, id, brief),
  materialize: (jobId: string, id: string): Promise<PatternMeta> => ipcRenderer.invoke('pipeline:materialize', jobId, id),
