...
第一步 · 物理层（先别看图，只根据材质和几何推）。先写**材质的灵魂**：一句话说清什么决定了这个表面的全部明暗（例如「一层薄膜绷在形上，明暗全是厚度和曲率的函数」「厚实心块，明暗全是光源和法线的夹角」「表面粗糙，没有镜面，只有 AO 和底色」）。之后的通道摘要、图层、光源约定都必须能从这句话推出来。再按 skill 第 1 节写一段**通道摘要**（albedo / roughness / metalness / normal / AO / 透光），再推图层。先用一个材质词定性（玻璃 / 塑料薄片 / 亚克力 / 乳胶 / 金属 / 纸 / 果冻 …）并写出几何（薄壁筒、圆环、囊、平板 …），然后列出光学上"应该有"的层：镜面高光的锐度由表面粗糙度决定、菲涅尔带来的边缘压暗或边缘反光、次表面散射带来的透光暖色、薄处淡厚处浓的厚度着色、自阴影和接触阴影落在哪、卷边/圆环的内外高光位置由光源方向决定。每条写依据。最后定一个**光源约定**：方向、软硬、色温；demo 里所有高光和阴影都从这一个光源推，才会互相一致。
...
输出一张表，列：# | 层 | 通道 | 形状 | 位置 | 软硬 | 衰减方向 | 浓淡 | 叠加 | 来源 | 依据。表前依次写：材质定性一句话、**材质的灵魂一句话**、通道摘要一段、光源约定一行、可信区域几行。

第三步半 · 回验灵魂。层栈出来后反过来看：每一层能不能从那句灵魂推出来？在来源列之外加一列「推得出 / 推不出」。推不出的要么是环境噪音（丢），要么是灵魂写漏了；推不出的超过三层，就重写灵魂再对一遍，并在表前注明改了什么。
...
${refs.map((f) => `- ${f}`).join('\n')}
${crop ? `对照放大图：${crop}\n` : ''}
检查四件事：① 反馈要求的改动在截图里看得见吗；② 有没有改出穿帮（形状不闭合、层错位、旧形状残留、颜色发灰过曝）；③ 和参考图比，材质读法对不对（高光锐度、边缘压暗、透光感）；④ 多状态一致：材质要在每个状态下都读得通——光源没动、形状动了，高光和阴影要跟着表面走，hover 提亮是高光核心变亮不是整体加白。只有初始态对、别的状态不对，说明层是画上去的不是推出来的；⑤ 还像不像 spec 里 Material Layers 那句「材质的灵魂」说的东西：如果截图里的明暗不再由那条规则决定（比如乳胶的高光不再跟着曲率走），改回去，不要为了这一轮反馈牺牲整体材质。
